...
const importPath = "github.com/justjake/go-scripting/cli"
...
// {{ .HumanName}}UI is a CLI user interface for {{ .HumanName }}.
// This type was auto-generated by {{ .ImportPath }}.
type {{ .HumanName }}UI struct {
	// It's up to you to set 'UI.Implementation = &your{{ .HumanName }}{}' in your main().
	Impl {{ .Recv }}
	cli.UI
}

func (ui *{{ .HumanName }}UI) getCommandMethod(commandName string) (func(), bool) {
	switch commandName {
{{- range .UI.Commands }}
	case {{ printf "%q" .Name }}:
...
type uiparser struct {
	...
	*UI
	...
	// Type name, including *, of the reciever that we should discover commands
	// from.
	Recv string
}
...
func ToFileContents(ui *UI, recv string) string {
	...
	params := struct {
		...
		Serialized string
		Recv       string
		// Type name, without *, of the reciever
		HumanName string
	}{
		Package:    "main",
